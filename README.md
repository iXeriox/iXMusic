# iXMusic — streaming music player (Vue 3 + PHP REST API)
...
2. Configure environment variables (or edit `backend/config.php` directly):
   ...
   export DISCORD_CLIENT_ID=your_discord_application_id
   export DISCORD_CLIENT_SECRET=your_discord_application_secret
   export DISCORD_REDIRECT_URI=http://localhost:5173/auth/discord/callback
   ```
   ...
   Create the Discord application in the
   [Discord Developer Portal](https://discord.com/developers/applications),
   add the redirect URI above under **OAuth2**, and never expose the client
   secret in the frontend. Existing installations should also apply
   `backend/migrations/001_discord_oauth.sql` once.
...
| `/api/auth.php?action=discord` | POST | — | Exchange a Discord authorization code and create/update a local session. |
...
```bash
cd frontend
cp .env.example .env       # configure API URL, Discord client ID, and redirect URI
npm install
npm run dev
```

Open the printed local URL (default `http://localhost:5173`). Register an
account through Discord to get started.
...
- Synced and plain-text lyrics are fetched from
  [LRCLIB](https://lrclib.net), an open-source lyrics service. Availability
  varies by recording metadata; iXMusic does not scrape or store lyrics.
