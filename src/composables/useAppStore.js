...
export function useAppStore() {
  async function initialize() {
    ...
    try {
      const [auth, playlistData, likedData, historyData, appData] = await Promise.all([
        api('/auth.php?action=me'), api('/playlists.php'), api('/tracks.php?action=liked'), api('/tracks.php?action=history'), api('/app.php?action=settings')
      ])
  ...
  async function createPlaylist(name, isPublic = false) { const data = await api('/playlists.php', { method: 'POST', body: { name, is_public: isPublic } }); const item = normalizePlaylist(data.playlist); state.value.playlists.unshift(item); return item }
  ...
  async function searchYouTube(query, popular = false) { const data = await api(`/youtube.php?q=${encodeURIComponent(query)}${popular ? '&popular=1' : ''}`); return data.results.map(normalizeTrack) }
  async function loadManagement() { const [members, blocked] = await Promise.all([api('/users.php'), api('/app.php?action=blocked')]); state.value.users = members.users; state.value.blocked = blocked.blocked }
  async function updateUser(id, changes) { const data = await api(`/users.php?id=${id}`, { method: 'PUT', body: changes }); const index = state.value.users.findIndex(item => +item.id === +id); if (index >= 0) state.value.users[index] = data.user }
  async function blockTrack(track, reason) { await api('/app.php?action=blocked', { method: 'POST', body: { youtube_video_id: track.video, reason } }); track.blocked = true; track.blocked_message = reason; await loadManagement() }
  async function unblockTrack(videoId) { await api(`/app.php?action=blocked&video_id=${encodeURIComponent(videoId)}`, { method: 'DELETE' }); await loadManagement() }
  async function saveSettings(settings) { await api('/app.php?action=settings', { method: 'PUT', body: settings }); state.value.settings = { ...state.value.settings, ...settings } }
  async function removeUser(id) { await api(`/users.php?id=${id}`, { method: 'DELETE' }); state.value.users = state.value.users.filter(item => +item.id !== +id) }
  async function loadProfile(id) { return api(`/profiles.php?id=${id}`) }
