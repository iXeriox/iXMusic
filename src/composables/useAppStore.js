...
export function useAppStore() {
  async function initialize() {
    ...
    try {
      const [auth, playlistData, likedData, historyData] = await Promise.all([
        api('/api/auth.php?action=me'), api('/api/playlists.php'), api('/api/tracks.php?action=liked'), api('/api/tracks.php?action=history')
      ])
  ...
  async function createPlaylist(name) { const data = await api('/api/playlists.php', { method: 'POST', body: { name } }); const item = normalizePlaylist(data.playlist); state.value.playlists.unshift(item); return item }
