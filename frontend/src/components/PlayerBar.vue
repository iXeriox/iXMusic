...
<template>
  <footer class="player-bar">
    ...
    <div class="volume">
      <button class="lyrics-btn" :class="{ on: showLyrics }" :disabled="!player.currentTrack" @click="showLyrics = !showLyrics" title="Lyrics">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M5 4h14a2 2 0 012 2v9a2 2 0 01-2 2h-7l-4.5 3v-3H5a2 2 0 01-2-2V6a2 2 0 012-2zm2 4v2h10V8H7zm0 4v2h7v-2H7z"/></svg>
        <span>Lyrics</span>
      </button>
      <span class="waveform" :class="{ paused: !player.isPlaying }"><span /><span /><span /><span /></span>
      <input type="range" min="0" max="100" :value="player.volume" @input="onVolume" class="seek volume-slider" :style="{ '--progress': `${player.volume}%` }" />
    </div>
    <Transition name="lyrics"><LyricsPanel v-if="showLyrics" @close="showLyrics = false" /></Transition>
  </footer>
</template>
...
.seek {
  ...
  background: linear-gradient(to right, var(--accent-soft) 0 var(--progress, 0%), #34333e var(--progress, 0%) 100%);
  transition: height .18s ease;
}
.seek:hover { height: 6px; }
.seek::-webkit-slider-thumb {
  -webkit-appearance: none; width: 13px; height: 13px; border-radius: 50%;
  background: #fff; opacity: 1; transform:scale(.7); box-shadow:0 0 0 4px rgba(167,139,250,.12); transition: transform 0.15s;
}
.seek:hover::-webkit-slider-thumb { transform:scale(1); }
.seek::-moz-range-thumb { width:13px;height:13px;border:0;border-radius:50%;background:#fff;box-shadow:0 0 0 4px rgba(167,139,250,.12); }

.volume { display: flex; align-items: center; gap: 10px; justify-content: flex-end; }
.volume-slider { width: 90px; }
.lyrics-btn { height:32px;padding:0 10px;border:1px solid var(--border-subtle);border-radius:8px;background:transparent;color:var(--text-secondary);display:flex;align-items:center;gap:6px;font-size:11px;font-weight:700; }.lyrics-btn svg{width:15px}.lyrics-btn:hover,.lyrics-btn.on{color:var(--accent-soft);border-color:var(--accent-dim);background:rgba(124,92,255,.08)}.lyrics-btn:disabled{opacity:.35}.lyrics-enter-active,.lyrics-leave-active{transition:opacity .25s,transform .25s}.lyrics-enter-from,.lyrics-leave-to{opacity:0;transform:translateX(20px)}
@media(max-width:760px){.player-bar{grid-template-columns:1fr auto;padding:0 12px}.controls{position:absolute;left:12px;right:12px;bottom:6px}.transport{display:none}.scrubber{max-width:none}.volume-slider,.waveform,.lyrics-btn span{display:none}.volume{gap:4px}.now-playing{padding-bottom:20px}}
</style>
